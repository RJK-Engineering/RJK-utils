...
I don't think what you say makes any sense, in fact, some of it is false. How can you relate those horrible things directly to leftism? I really don't get it. As I said, the policies the left in your country wants are mainstream in my country and we're doing fine, in fact, we're doing better in just about any measureable way,

Saying those horrible .. result of leftism is, if anything, the regimes you talk about were more to the right, but in name of leftis (this is a common theme in history, for example, the nazi's called themselves socialist but were right wing nationalists, north korea call themselves democratic but are totalitarian, the same was true for the ddr, ..)


I don't think what you say makes any sense, in fact, some of it is false. How can you relate those horrible things directly to leftism? I really don't get it. If anything, the regimes you talk about were more to the right, but in name of leftis (this is a common theme in history, for example, the nazi's called themselves socialist but were right wing nationalists, north korea call themselves democratic but are totalitarian, the same was true for the DDR, ..).
But you can't say a certain political stromings are responsible for these atrocities, as there are plenty of left wing countries that are doing great. In fact, those are the countries with the highest living standards.


As I said, the policies the left in your country wants are mainstream in my country and we're doing fine, in fact, we're doing better in just about any measureable way,

Saying those horrible .. result of leftism is,

-----------------

    _callAnnotationService: function(serviceName, annotationClassNames, items, callback) {
      var repository = this.caseFolder.repository;
      var caseFolderId = this.caseFolder.attributes.Id;
      var requestBody = {};

      for (var i=0; i<annotationClassNames.length; i++) {
        var annotationClassName = annotationClassNames[i];

        requestBody[annotationClassName] = {
          // annotationClass: annotationClassName,
          caseFolderId: caseFolderId,
          documentVersionIdList: items.map(i => i.vsId),
          documentVersionStatus: 0
        };
      }

      // var aClass = this.getAnnotationClass(annotationClassName);
      // var vsIdList = [];

      // for (var i=0; i<items.length; i++) {
      //  var vsId = items[i].vsId;
      //  vsIdList.push(vsId);
      // }
      ...
      //  annotationClass: annotationClassName,
      //  caseFolderId: caseFolderId,
      //  documentVersionIdList: vsIdList,
      //  documentVersionStatus: 0
      // };


      // previous/new
      // response = { status: "", vsId: state, hasDocumentInformation: true }
      // response = { status: "", annotations: {vsId: state} }
      ServiceUtils.callService(repository, serviceName, requestBody, function(response) {
        response.annotations.forEach(function (annotation) {
          var aClass = this.getAnnotationClass(annotation.className);
          aClass.response = annotation;
          aClass.response.status = response.status;
        });
        if (callback) callback(response);
      }, true);

      // ServiceUtils.callService(repository, serviceName, requestBody, function(response) {
      //  aClass.response = response;
